...
async def follow(self: BaseBot, user: User, message: str) -> None:
    async def following_loop(self: BaseBot, user: User, message: str) -> None:
        if message.startswith("/following_loop"):
            await self.highrise.chat("Invalid command, use /follow")
            return
        while True:
    ...
    for task in task_list:
        if task.get_name() == "following_loop":
            await self.highrise.chat("I'm already following someone ")
            return
    ...
    await self.highrise.chat(f"I'm Following   {user.username} 🚶‍♂️")
...
async def stop(self: BaseBot, user: User, message: str) -> None:
    ...
    for task in task_list:
        if task.get_name() == "following_loop":
            task.cancel()
            await self.highrise.chat(f"Unfollowing   {user.username}")
            return
    await self.highrise.chat("I'm not following anyone  ")
    return